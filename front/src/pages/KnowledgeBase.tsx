...
export default function KnowledgeBase() {
  ...
  const [refreshKey, setRefreshKey] = useState(0) // 手动刷新触发器
  const pageSize = 20
  ...
  // ── 加载文档列表（当 selectedId / page / search / statusFilter 变化时） ──
  useEffect(() => {
    let cancelled = false
    ...
    return () => {
      cancelled = true // 组件卸载或 deps 变化时取消上一次的请求
    }
  ...
  const handleFilesSelected = async (files: FileList) => {
    console.log('[上传] 文件已选择:', files.length, '个文件')
    if (!selectedId) {
      toast.error(t('knowledge.noKBForChat'))
      return
    }
    const fileList = Array.from(files).filter((f) => {
      const ext = f.name.split('.').pop()?.toLowerCase()
      if (!ext || !['pdf', 'txt', 'md', 'docx', 'pptx'].includes(ext)) {
        toast.error(`不支持的文件类型: ${f.name}`)
        return false
      }
      if (f.size > 200 * 1024 * 1024) {
        toast.error(`文件太大: ${f.name} (超过 200MB)`)
        return false
      }
      return true
    })
    ...
    try {
      ...
      for (let i = 0; i < fileList.length; i += 10) {
        ...
        for (const f of batch) {
          results.push({
            file: f,
            status: failedSet.has(f.name) ? 'fail' : 'success',
            error: failMap.get(f.name),
          })
        }
      }

      setUploadFiles(results.map((r) => ({
        file: r.file,
        status: r.status,
        error: r.error,
      })))
    ...
    // 10 秒后自动清除上传结果提示（不论成功或失败）
    setTimeout(() => setUploadFiles([]), 10_000)
  }

  const dismissUploadResult = (index: number) => {
  ...
  const handleDeleteDoc = async () => {
    ...
    try {
      const res = await knowledgeApi.deleteById(selectedId, deleteTarget.id)
  ...
  const handleCreateKB = async () => {
    ...
    setCreatingKB(true)
    try {
      await create(newKBName.trim(), newKBDesc.trim() || undefined)
  ...
  const formatDate = (dateStr: string) =>
    new Date(dateStr).toLocaleDateString('zh-CN', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-6">
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        ...
        <h1 className="font-heading text-xl font-semibold text-[var(--color-text)] mr-2">
          {t('knowledge.title')}
        </h1>
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 text-sm rounded-md bg-[var(--color-accent)] text-white hover:bg-blue-700 transition-colors cursor-pointer shrink-0"
        >
          <Plus size={14} />
        ...
        {selectedKB && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-sm rounded-md border border-[var(--color-border)] text-[var(--color-danger)] hover:bg-[var(--color-danger-bg)] transition-colors cursor-pointer shrink-0"
          >
            <Trash2 size={14} />
            删除
          </button>
      ...
      {selectedKB?.description && (
        <p className="text-xs text-[var(--color-text-tertiary)] mb-4 -mt-3">
          {selectedKB.description}
        </p>
      )}
      ...
      {selectedId ? (
        <>
          ...
          >
            <input
              ...
              className="hidden"
              onChange={(e) => {
                console.log('[上传] onChange 触发', e.target.files?.length)
                if (e.target.files) handleFilesSelected(e.target.files)
                // 重置 value 确保同名文件可重复选择
                e.target.value = ''
              }}
            />
            ...
            <label
              ...
              className="inline-block px-4 py-2 text-sm rounded-md bg-[var(--color-accent)] text-white hover:bg-blue-700 transition-colors cursor-pointer"
            >
              {t('knowledge.upload')}
            </label>
          </div>
          ...
          {uploadFiles.length > 0 && (
            <div className="mt-4 space-y-2">
              {uploadFiles.map((uf, i) => (
                <div key={i} className="flex items-center gap-3 px-4 py-3 rounded-lg bg-[var(--color-card)] border border-[var(--color-border)]">
                  ...
                  {uf.status === 'fail' && uf.error && (
                    <span className="text-xs text-[var(--color-danger)] max-w-[200px] truncate" title={uf.error}>{uf.error}</span>
                  )}
                  {uf.status !== 'pending' && uf.status !== 'uploading' && (
                    <button
                      onClick={() => dismissUploadResult(i)}
                      className="p-0.5 rounded text-[var(--color-text-tertiary)] hover:text-[var(--color-danger)] hover:bg-[var(--color-danger-bg)] transition-colors shrink-0 cursor-pointer"
                      title="关闭"
                    >
              ...
              {!uploading && uploadFiles.every((f) => f.status === 'success') && (
          ...
          <div className="mt-4">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-medium text-[var(--color-text)]">
                ...
                <span className="text-[var(--color-text-tertiary)] ml-1">
                  ({t('knowledge.total')} {total} {t('knowledge.items')})
                </span>
              </h2>
              <button
                onClick={() => setRefreshKey((k) => k + 1)}
                className="p-1.5 rounded text-[var(--color-text-tertiary)] hover:text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-secondary)] transition-colors"
            ...
            {loading ? (
              ...
              <EmptyState
                ...
                message={search || statusFilter ? '没有符合条件的文档' : t('knowledge.empty')}
              />
            ) : (
              <>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0 ml-3">
                        <span
                          ...
                          style={{ color: STATUS_BADGE[doc.status]?.color, backgroundColor: STATUS_BADGE[doc.status]?.bg }}
                        >
        ...
        <EmptyState
          ...
          message={t('knowledge.noKB')}
          action={
            <button
              onClick={() => setShowCreateKB(true)}
              className="px-4 py-2 text-sm rounded-md bg-[var(--color-accent)] text-white hover:bg-blue-700 transition-colors cursor-pointer"
            >
      ...
      <ConfirmDialog
        open={showDeleteKB}
        onOpenChange={setShowDeleteKB}
        title={t('common.confirm')}
        message={t('knowledge.deleteKBConfirm')}
        variant="danger"
        confirmText={t('common.delete')}
      ...
      <Dialog.Root open={showCreateKB} onOpenChange={setShowCreateKB}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] max-w-[90vw] bg-[var(--color-card)] rounded-xl shadow-xl p-6">
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button
                ...
                className="px-4 py-2 text-sm rounded-md bg-[var(--color-accent)] text-white hover:bg-blue-700 disabled:opacity-40 transition-colors cursor-pointer"
              >
